...
stopdate<-as.Date("2030-12-31") #defult ("2020-12-31)
...
date_vaccine_on<-as.Date("2020-03-11")
...
parameters <- c(
  ...
  p=0.042/3,           # probabilty of infection given a contact 
  ...
  vaccine_eff1=90,   # vaccine efficacy (%)- min 0 max 100 step 1; reduce infection
  ...
  vaccine_cov=80,    # vaccine coverage (%)- min 0 max 100 step 1
  ...
  with(as.list(c(Y, parameters)),
       {
         ...
         # ODE system
         dSdt <- -S*lam-S*vaccinate+omega*R+ageing%*%S-mort*S+birth-quarantine_rate*S +(1/quarantine_days)*QS+omegav*SV
         dEdt <- S*lam-gamma*E+ageing%*%E-mort*E-quarantine_rate*E+(1/quarantine_days)*QE # + (1-vaccine_eff)*lam*V 
         ...
         dSVdt <- S*vaccinate-(1-vaccine_eff1)*lam*SV+ageing%*%SV-mort*SV-omegav*SV #Assuming the lam is the same as general population
         dEVdt <- (1-vaccine_eff1)*lam*SV-gamma*EV +ageing%*%EV-mort*EV
         ...
         + nus*(1-pdeath_h*ifr[,2])*HV + (1-pdeath_icu*ifr[,2])*nu_icu*ICU + (1-pdeath_icuc*ifr[,2])*nu_icuc*ICUC + (1-pdeath_hc*ifr[,2])*nusc*HC + (1-pdeath_vent*ifr[,2])*nu_vent*Vent+ (1-pdeath_ventc*ifr[,2])*nu_ventc*VentC-omega*RV
